...
export class CanteenRbacService {
  ...
  async createUser(dto: CreateCanteenUserDto, actorUserId?: string) {
    ...
    let systemRoleId = dto.roleId;
    if (systemRoleId) {
      const sysRole = await this.prisma.role.findUnique({ where: { id: systemRoleId } });
      if (!sysRole) {
        throw new NotFoundException('Specified system role not found.');
      }
    } else {
      ...
      systemRoleId = fallbackRole.id;
    }

    if (dto.canteenRoleId) {
      const canteenRole = await this.prisma.canteenRole.findUnique({
        where: { id: dto.canteenRoleId },
      });
      if (!canteenRole) {
    ...
    await this.auditService.log({
      ...
      metadata: { name: user!.name, email: user!.email, canteenRoleId: dto.canteenRoleId },
    });
