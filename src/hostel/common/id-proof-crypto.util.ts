 ...
 *
 * The key comes from HOSTEL_ID_PROOF_KEY only — deliberately no fallback to
 * JWT_SECRET, so rotating a login secret can never make stored ID proofs
 * unreadable. A missing key fails closed.
 */
function resolveKey(): Buffer {
  const raw = process.env.HOSTEL_ID_PROOF_KEY;
  if (!raw) {
    throw new InternalServerErrorException(
      'HOSTEL_ID_PROOF_KEY must be set to store visitor ID proofs',
    );
  }
...
/** Last-4 view of an ID proof number, safe to show to any viewer. */
