...
@Injectable()
export class HostelDynamicRolesService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly permissionsService: HostelPermissionsRegistryService,
  ) {}
  ...
  private async assertRulesInCatalog(rules: RuleInput[]) {
    const active = await this.prisma.hostelPermission.findMany({
  ...
  async createRole(actor: HostelPlatformUser, dto: CreateHostelDynamicRoleDto) {
    this.requireInstituteAdmin(actor);
    await this.assertRulesInCatalog(dto.permissions);
    const existing = await this.prisma.hostelDynamicRole.findUnique({
      where: {
    ...
    return this.prisma.hostelDynamicRole.create({
      data: {
  ...
  async listRoles(actor: HostelPlatformUser) {
    this.requireInstituteAdmin(actor);
    return this.prisma.hostelDynamicRole.findMany({
   ...
   */
  async getRole(actor: HostelPlatformUser, roleId: number) {
    this.requireInstituteAdmin(actor);
    const role = await this.prisma.hostelDynamicRole.findFirst({
  ...
  async updateRole(
    ...
    return this.prisma.hostelDynamicRole.update({
  ...
  async deleteRole(actor: HostelPlatformUser, roleId: number) {
    this.requireInstituteAdmin(actor);
    const role = await this.getRole(actor, roleId);
    ...
    return this.prisma.hostelDynamicRole.delete({
      where: { role_id: roleId },
    });
  }
  ...
  async assignUser(actor: HostelPlatformUser, dto: AssignHostelUserToRoleDto) {
    this.requireInstituteAdmin(actor);
    ...
    const usernameOwner = await this.prisma.hostelUserDynamicRole.findUnique({
      where: {
    ...
    const existing = await this.prisma.hostelUserDynamicRole.findUnique({
      where: {
    ...
    const saved = existing
      ? await this.prisma.hostelUserDynamicRole.update({
      ...
      : await this.prisma.hostelUserDynamicRole.create({
          data: {
            institute_id: actor.institute_id,
            eddva_user_id: dto.eddva_user_id,
            user_name: dto.user_name,
  ...
  async listUserAssignments(actor: HostelPlatformUser) {
    this.requireInstituteAdmin(actor);
    const list = await this.prisma.hostelUserDynamicRole.findMany({
      where: { institute_id: actor.institute_id },
      include: { role: true },
  ...
  async revokeUser(actor: HostelPlatformUser, assignmentId: number) {
    this.requireInstituteAdmin(actor);
    const assignment = await this.prisma.hostelUserDynamicRole.findFirst({
      where: { id: assignmentId, institute_id: actor.institute_id },
    });
    if (!assignment)
      throw new NotFoundException(`Assignment #${assignmentId} not found`);
    const deleted = await this.prisma.hostelUserDynamicRole.delete({
      where: { id: assignmentId },
    });
  ...
  async getMyPermissions(actor: HostelPlatformUser) {
  ...
  private requireInstituteAdmin(actor: HostelPlatformUser) {
    if (!isHostelAdmin(actor)) {
      throw new ForbiddenException(
        'Only Institute Admin can manage Hostel roles and permissions',
      );
    }
  }
}
