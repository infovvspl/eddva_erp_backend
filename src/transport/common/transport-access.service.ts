...
export function isTransportAdmin(actor: TransportPlatformUser): boolean {
  return actor.is_institute_admin || actor.user_role === 'INSTITUTE_ADMIN';
}

/**
 * Resolves the acting Transport Platform user's permission matrix. Mirrors
 * FrontOfficeAccessService/InventoryAccessService's live-lookup logic
...
export class TransportAccessService {
  ...
  async getPermissionRules(actor: TransportPlatformUser): Promise<{ rules: PermissionRule[]; roleName: string }> {
    let rules: PermissionRule[] = [];
    let roleName = actor.role_name || actor.user_role || 'Assigned User';

    if (actor.role_id) {
      const role = await this.prisma.transportDynamicRole.findUnique({ where: { role_id: actor.role_id } });
  ...
  async hasPermission(actor: TransportPlatformUser, resource: string, action: string): Promise<boolean> {
    if (isTransportAdmin(actor)) return true;
