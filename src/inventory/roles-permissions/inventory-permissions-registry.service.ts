...
import { INVENTORY_RESOURCE_CATALOG } from './dto/create-dynamic-role.dto';

@Injectable()
export class InventoryPermissionsRegistryService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      console.error('Failed to seed default Inventory permissions:', err);
    }
  }

  /** Auto-seeds system default permissions into the inventory_permissions_catalog table */
  async seedDefaultPermissions() {
    for (const item of INVENTORY_RESOURCE_CATALOG) {
      for (const action of item.available_actions) {
        const key = `${item.resource}:${action}`;
        const name = `${action.toUpperCase()} ${item.name}`;

        const existing = await this.prisma.inventoryPermission.findUnique({ where: { key } });

        if (!existing) {
          await this.prisma.inventoryPermission.create({
            data: {
              key,
  ...
  async listPermissions() {
    const list = await this.prisma.inventoryPermission.findMany({
  ...
  async getPermission(id: number) {
    const perm = await this.prisma.inventoryPermission.findUnique({ where: { permission_id: id } });
    if (!perm) throw new NotFoundException(`Permission #${id} not found`);
    return perm;
  }

  async createPermission(dto: CreateInventoryCustomPermissionDto) {
    ...
    return this.prisma.inventoryPermission.create({
      data: {
        key,
  ...
  async updatePermission(id: number, dto: UpdateInventoryCustomPermissionDto) {
    await this.getPermission(id);
    return this.prisma.inventoryPermission.update({
  ...
  async deletePermission(id: number) {
    ...
    return this.prisma.inventoryPermission.delete({ where: { permission_id: id } });
  }
}
