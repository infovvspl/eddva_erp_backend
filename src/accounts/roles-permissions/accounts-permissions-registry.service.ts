...
import { ACCOUNTS_RESOURCE_CATALOG } from './dto/create-dynamic-role.dto';

@Injectable()
export class AccountsPermissionsRegistryService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      console.error('Failed to seed default Accounts permissions:', err);
    }
  }

  /** Auto-seeds system default permissions into the accounts_permissions_catalog table */
  async seedDefaultPermissions() {
    for (const item of ACCOUNTS_RESOURCE_CATALOG) {
      for (const action of item.available_actions) {
        const key = `${item.resource}:${action}`;
        const name = `${action.toUpperCase()} ${item.name}`;

        const existing = await this.prisma.accountsPermission.findUnique({ where: { key } });

        if (!existing) {
          await this.prisma.accountsPermission.create({
            data: {
              key,
  ...
  async listPermissions() {
    const list = await this.prisma.accountsPermission.findMany({
  ...
  async getPermission(id: number) {
    const perm = await this.prisma.accountsPermission.findUnique({ where: { permission_id: id } });
    if (!perm) throw new NotFoundException(`Permission #${id} not found`);
    return perm;
  }

  async createPermission(dto: CreateAccountsCustomPermissionDto) {
    ...
    return this.prisma.accountsPermission.create({
      data: {
        key,
  ...
  async updatePermission(id: number, dto: UpdateAccountsCustomPermissionDto) {
    await this.getPermission(id);
    return this.prisma.accountsPermission.update({
  ...
  async deletePermission(id: number) {
    ...
    return this.prisma.accountsPermission.delete({ where: { permission_id: id } });
  }
}
