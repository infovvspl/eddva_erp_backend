...
import { ADMISSION_RESOURCE_CATALOG } from './admission-permission-catalog';
...
@Injectable()
export class AdmissionPermissionsRegistryService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      console.error('Failed to seed default Admission permissions:', err);
    }
  }

  /**
   * Auto-seeds system default permissions into admission_permissions_catalog in
   * one idempotent createMany({ skipDuplicates: true }) — the unique `key`
   * constraint makes Postgres skip rows that already exist.
   */
  async seedDefaultPermissions() {
    const data = ADMISSION_RESOURCE_CATALOG.flatMap((item) =>
    ...
    await this.prisma.admissionPermission.createMany({
      data,
      skipDuplicates: true,
    });
  }
  ...
  async listPermissions() {
    const list = await this.prisma.admissionPermission.findMany({
  ...
  async getPermission(id: number) {
    const perm = await this.prisma.admissionPermission.findUnique({
  ...
  async createPermission(
    ...
    dto: CreateAdmissionCustomPermissionDto,
  ) {
    ...
    const existing = await this.prisma.admissionPermission.findUnique({
      where: { key },
    });
    if (existing) {
    ...
    return this.prisma.admissionPermission.create({
      data: {
        key,
        resource,
        action,
  ...
  async updatePermission(
    actor: AdmissionPlatformUser,
    id: number,
    dto: UpdateAdmissionCustomPermissionDto,
  ) {
    this.requireInstituteAdmin(actor);
    await this.getPermission(id);
    return this.prisma.admissionPermission.update({
      where: { permission_id: id },
      data: {
  ...
  async deletePermission(actor: AdmissionPlatformUser, id: number) {
    this.requireInstituteAdmin(actor);
    ...
    return this.prisma.admissionPermission.delete({
      where: { permission_id: id },
    });
  }
  ...
  private requireInstituteAdmin(actor: AdmissionPlatformUser) {
    if (!isAdmissionAdmin(actor)) {
      throw new ForbiddenException(
        'Only Institute Admin can manage the Admission permission catalog',
      );
    }
  }
}
