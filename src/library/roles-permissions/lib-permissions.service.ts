...
import { LIBRARY_RESOURCE_CATALOG } from './dto/create-dynamic-role.dto';

@Injectable()
export class LibPermissionsService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      console.error('Failed to seed default Library permissions:', err);
    }
  }

  /** Auto-seeds system default permissions into library_permissions DB table */
  async seedDefaultPermissions() {
    for (const item of LIBRARY_RESOURCE_CATALOG) {
      for (const action of item.available_actions) {
        const key = `${item.resource}:${action}`;
        const name = `${action.toUpperCase()} ${item.name}`;

        const existing = await this.prisma.libPermission.findUnique({
          where: { key },
        });

        if (!existing) {
          await this.prisma.libPermission.create({
            data: {
              key,
  ...
  async listPermissions() {
    const list = await this.prisma.libPermission.findMany({
      orderBy: [{ category: 'asc' }, { name: 'asc' }],
    });

    // Group permissions by resource for dynamic UI matrix rendering
  ...
  async getPermission(id: number) {
    const perm = await this.prisma.libPermission.findUnique({ where: { permission_id: id } });
    if (!perm) throw new NotFoundException(`Permission #${id} not found`);
    return perm;
  }

  async createPermission(dto: CreateCustomPermissionDto) {
    const key = `${dto.resource.trim().toLowerCase()}:${dto.action.trim().toLowerCase()}`;
    const existing = await this.prisma.libPermission.findUnique({ where: { key } });
    if (existing) {
      throw new ConflictException(`Permission with key "${key}" already exists in database`);
    }

    return this.prisma.libPermission.create({
      data: {
        key,
  ...
  async updatePermission(id: number, dto: UpdateCustomPermissionDto) {
    await this.getPermission(id);
    return this.prisma.libPermission.update({
  ...
  async deletePermission(id: number) {
    ...
    return this.prisma.libPermission.delete({ where: { permission_id: id } });
  }
}
